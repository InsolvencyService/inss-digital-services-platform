...
public class SearchTermFlowNodeExecutorTests
{
    private readonly SearchTermFlowNodeExecutor _searchTermFlowNodeExecutor = new();
    ...
    public SearchTermFlowNodeExecutorTests()
    {
    ...
    [Fact]
    public async Task PerformingSearching_ExecuteAsync_ReturnsSameNode()
    {
        ...
        NodeId? nextNodeId = await _searchTermFlowNodeExecutor.ExecuteAsync(context);
    ...
    public async Task PerformingSearchWithSingleWord_ExecuteAsync_AddsNewSearchKeywordQueryParam()
    {
        ...
        await _searchTermFlowNodeExecutor.ExecuteAsync(context);

        string? keyword = context.GetQueryParam<string>("keyword");
        Assert.NotNull(keyword);
        Assert.Equal("Test", keyword);
    }
    ...
    public async Task PerformingSearchWithMultipleWords_ExecuteAsync_AddsNewEncodedSearchKeywordQueryParam()
    {
        FlowNode searchResultNode = _flowchart.Nodes.First(n => n.Id == _searchResultId);
        SearchTermModel searchTerm = _section.Pages.GetFirstOf<SearchTermModel>();
        searchTerm.SearchText = "Test Team";
        FlowNodeContext context = new()
        { 
            Nodes = _flowchart.Nodes,
            CurrentNode = searchResultNode,
