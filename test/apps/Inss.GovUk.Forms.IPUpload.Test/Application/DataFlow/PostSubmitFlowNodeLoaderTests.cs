...
public class PostSubmitFlowNodeLoaderTests
{
    ...
    public PostSubmitFlowNodeLoaderTests()
    {
        ...
        _postSubmitFlowNodeLoader = new PostSubmitFlowNodeLoader(_userFormService, _uploadContentBlobClient);
    }
    
    [Fact]
    public async Task LoadingPostSubmit_ExecuteAsync_RemovesXml()
    {
        ...
        await _uploadContentBlobClient.Received(1).RemoveAsync(context.Form.Id);
    }
    
    [Fact]
    public async Task LoadingPostSubmit_ExecuteAsync_RemovesForm()
    {
        FormModel form = TestFormModels.CreateWithIPUploadSection();
        SectionModel section = form.Sections["IP Upload"];
        PostSubmitModel postSubmit = section.Pages.GetFirstOf<PostSubmitModel>();
