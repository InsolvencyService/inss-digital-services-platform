...
public class SessionController : Controller
{
    ...
    {
        string openIdConnectScheme = Request.Query["login_hint"].ToString();
        string postLogoutRedirectUri = Request.Query["post_logout_redirect_uri"].ToString();
        
        if (openIdConnectScheme.Equals("onelogin", StringComparison.OrdinalIgnoreCase) ||
            openIdConnectScheme.Equals("entra", StringComparison.OrdinalIgnoreCase))
        {
            _logger.SkippingSchemeLogout(openIdConnectScheme, postLogoutRedirectUri);
