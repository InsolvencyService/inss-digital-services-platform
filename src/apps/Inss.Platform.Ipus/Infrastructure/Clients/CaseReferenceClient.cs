...
public sealed partial class CaseReferenceClient : ICaseReferenceClient
{
    ...
    public async Task<CaseReferenceResponse?> LookupCaseDetails(CaseReferenceRequest request)
    {
        string filter = $"$select=inss_name&$filter=contains(inss_name,'{request.CaseReference}')";
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            {
                return new CaseReferenceResponse(request.CaseReference, caseMessage.CompanyName);
            }
        }
