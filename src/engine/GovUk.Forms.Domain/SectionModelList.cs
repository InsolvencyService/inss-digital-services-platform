...
public sealed class SectionModelList : List<SectionModel>
{
    ...
    public SectionModel? FindSection(ContentPath path)
    {
        ContentPath? sectionPath = path.FindSection();
        return sectionPath is not null ? this.FirstOrDefault(s => s.Path == sectionPath) : null;
    }
}