...
/// </summary>
/// <typeparam name="T">The entity type, which must inherit from <see cref="AuditableEntity"/>.</typeparam>
public abstract class RepositoryBase<T> : IRepositoryBase<T> where T : AuditableEntity
{
    ...
    protected RepositoryBase(ILogger<RepositoryBase<T>> logger, UserManagementDbContext context)
    {
        _logger = logger;
        _context = context;
    }
